defmodule YahtzeeLowerSectionTest do
  ...
  test "Identify 'Small straight'" do
    Enum.map(1..6, fn _ ->
      a = Enum.shuffle([1, 2, 3, 4, Enum.random([1, 2, 3, 4, 6])])
      b = Enum.shuffle([2, 3, 4, 5, Enum.random([2, 3, 4, 5])])
      c = Enum.shuffle([3, 4, 5, 6, Enum.random([1, 3, 4, 5, 6])])

      seq = Enum.random([a, b, c]) |> Enum.shuffle()
  ...
  test "Identify 'Large straight'" do
    Enum.map(1..6, fn _ ->
      a = Enum.shuffle([1, 2, 3, 4, 5])
      b = Enum.shuffle([2, 3, 4, 5, 6])

      seq = Enum.random([a, b]) |> Enum.shuffle()
      assert %{"Large straight": 40} = Yahtzee.score_lower(seq)
    end)
  end

  test "Identify dice configuration of 2 twos and 3 Fives" do
    seq = [2, 2, 5, 5, 5] |> Enum.shuffle
    sum = Enum.sum(seq)
  ...
  test "Identify a combination with no duplicates and no straights" do
    seq = [1, 2, 4, 5, 6] |> Enum.shuffle
    sum = Enum.sum(seq)
  ...
  test "Identify a combination with no chance and a small straight" do
