defmodule Yahtzee do
  ...
  def score_lower(dice) do
    ...
    %{
      ...
      "Full house":
        if face_frequencies |> Enum.member?(3)
        and face_frequencies |> Enum.member?(2) do 25 else 0 end,
      "Yahtzee":
        if face_frequencies |> Enum.member?(5) do 50 else 0 end,
      "Chance":
        if Enum.sort(face_frequencies) == [1, 2, 2]
        or ((face_frequencies == [1, 1, 1, 1, 1] or face_frequencies == [1, 1, 1, 2]) and length(straight_roll) < 4) do sum else 0 end,
      "Small straight":
